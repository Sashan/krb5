...
#if 0
    /*
     * Ssshan: more work is required here (or relevant area)
     * the patch 019-log-rotation.patch Used to deliver this
     * chunk of code here:
@@ -262,6 +400,8 @@ klog_com_err_proc(const char *whoami, long int code, const char *format, va_list
...
+            klog_rotate(&log_control.log_entries[lindex]);
       ...
       We need to figure out where to put it. The function here
       used to be very different in 1.16.
     */
#endif
}
